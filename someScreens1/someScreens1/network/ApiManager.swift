...
class ApiManager {
    ...
    public func  getUsers(curPageNumber: Int, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        var urlString =  String(format: "%@/users?page=%@&per_page=%@", ApiManager.baseURL, String(describing:curPageNumber), String(describing:ApiManager.perPage))
    ...
    private func treatmentUsers(usersAr: [Dictionary<String, AnyObject>]) throws {
            ...
            do {
                ...
                if let fetchedObjects = try managedObjectContext.fetch(request) as? [UserCur] {
                    if fetchedObjects.count == 0 {
                        ...
                        curUser?.positionID = Int64(Int(el["positionID"]?.int64Value ?? 1))
    ...
    public func  getPositions(completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
    ...
    public func treatmentPositions(positionsAr: [Dictionary<String, AnyObject>]) throws {
        //increase name up to usersArray.count
        let requestIncrease = NSFetchRequest<NSFetchRequestResult>(entityName: "Position")
        let titleSort = NSSortDescriptor(key: "id", ascending: true)
        requestIncrease.sortDescriptors = [titleSort]
        let managedObjectContext =
            CoredataStack.mainContext
            ...
            do {
                ...
                if let fetchedObjects = try managedObjectContext.fetch(request) as? [UserCur] {
                    if fetchedObjects.count == 0 {
                        ...
                        let curUser = NSEntityDescription.insertNewObject(forEntityName: "Position", into: managedObjectContext) as? Position
                        curUser?.id =  Int64(Int(el["id"]?.int64Value ?? 0))
                        curUser?.name = el["name"] as? String
    ...
    public func  setUser(curPageNumber: Int, completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
        var urlString =  String(format: "%@/user?page=%@&per_page=%@", ApiManager.baseURL, String(describing:curPageNumber), String(describing:ApiManager.perPage))
        SwiftyBeaver.debug("urlString = \(urlString)")
        guard let url = URL(string: urlString) else {
            SwiftyBeaver.debug(urlString)
            return
    ...
    public func  getToken(completion: @escaping (Data?, URLResponse?, Error?) -> ()) {
    ...
    public func registerUser(name: String,
            ...
            guard let base64String = UserDefaults.standard.string(forKey: "token") else {
...
//            )
        ...
        SwiftyBeaver.debug("registerUser body: \(jsonData)")
            request.httpBody = jsonData
        SwiftyBeaver.debug("registerUser String(describing: request.allHTTPHeaderFields): \(request.debugDescription)")
        SwiftyBeaver.debug("registerUser String(describing: request.allHTTPHeaderFields): \(String(describing: request.allHTTPHeaderFields))")
        SwiftyBeaver.debug("registerUser String(describing: request.httpBody): \(String(describing: request.httpBody))")
            // Make the request
            let task = URLSession.shared.dataTask(with: request) { data, response, error in
                SwiftyBeaver.debug("Registration data: \(String(describing: data))")
                SwiftyBeaver.debug("Registration response: \(String(describing: response))")
                SwiftyBeaver.debug("Registration error: \(String(describing: error))")
                if let error = error {
                ...
                guard let httpResponse = response as? HTTPURLResponse else {
                    SwiftyBeaver.debug("Registration failed httpResponse: \(String(describing: error))")
    ...
    private func createRequestBody(parameters: [String: String], boundary: String, photoData: Data) -> Data {
            var body = Data()
